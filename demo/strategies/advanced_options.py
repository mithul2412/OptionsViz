...
Functions in this module expect pandas DataFrames with standard yfinance option chain structure.
"""
...
def calculate_max_pain(calls_df: pd.DataFrame, puts_df: pd.DataFrame) -> float:
    ...
    Args:
        calls_df: DataFrame containing call options data with 'strike' and 'openInterest' columns
        puts_df: DataFrame containing put options data with 'strike' and 'openInterest' columns
        
    Returns:
        float: The max pain strike price, or None if calculation fails
        
    Example:
        >>> calls_df = pd.DataFrame({'strike': [100, 105, 110], 'openInterest': [500, 300, 200]})
        >>> puts_df = pd.DataFrame({'strike': [90, 95, 100], 'openInterest': [200, 400, 300]})
...
def compute_iv_skew(calls_df: pd.DataFrame, current_price: float) -> dict:
    ...
    Args:
        calls_df: DataFrame containing call options with 'strike' and 'impliedVolatility' columns
        current_price: Current price of the underlying asset
        
    Returns:
        dict: Dictionary with keys 'itm', 'atm', 'otm' and corresponding IV values as percentages.
