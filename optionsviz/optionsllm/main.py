    ...
    for exp, exp_data in data["expirations"].items():
        ...
        for call in exp_data["calls"]:
            lines.append(f"     Strike={call['strike']} ({call['moneyness']}) IV={call['iv']}%, "
                         f"Delta={call['delta']}, Gamma={call['gamma']}, "
                         f"Theta={call['theta']}, Vega={call['vega']}, "
        ...
        for put in exp_data["puts"]:
            lines.append(f"     Strike={put['strike']} ({put['moneyness']}) IV={put['iv']}%, "
                         f"Delta={put['delta']}, Gamma={put['gamma']}, "
                         f"Theta={put['theta']}, Vega={put['vega']}, "
...
def ask_llm_about_options(
    ...
    Example:
        >>> data = build_compact_options_json("AAPL")
        >>> summary = summarize_options_data(data)
        >>> analysis = ask_llm_about_options(summary, "What strategy?")
    """
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description="Modular Options Analysis")
    ...
    parser.add_argument("--interactive", action="store_true",
                        help="Enter interactive LLM query mode")
    args = parser.parse_args()

    # Build the options data
    data = build_compact_options_json(args.ticker, expirations_limit=args.expirations,
                                      include_hv=args.hv)
    summary = summarize_options_data(data)
